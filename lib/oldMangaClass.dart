...
class OldMangaClass implements Comparable<OldMangaClass> {
  const OldMangaClass({
    required this.uuid,
    required this.title,
  ...
  factory OldMangaClass.fromJson(dynamic js) {
    print("WARING! To be deprecated soon!");
    OldMangaClass m = OldMangaClass(
        uuid: js['uuid'],
        title: js['title'],
  ...
  factory OldMangaClass.fromFirebase(
    DocumentSnapshot<Map<String, dynamic>> snapshot,
    SnapshotOptions? options,
  ) {
    final data = snapshot.data();
    return OldMangaClass(
        uuid: snapshot.id,
        title: data!['volumeTitle'] ?? emptyManga.title,
        titleOriginal: data['volumeTitleOriginal'] ?? emptyManga.titleOriginal,
        genres: [data['volumeGenre']],
        story: data['volumeAuthor'] ?? emptyManga.story,
        design: data['volumeMangaka'] ?? emptyManga.design,
        editor: data['volumeEditor'] ?? emptyManga.editor);
  }
  ...
  @override
  int compareTo(OldMangaClass other) {
    if (uuid == other.uuid) {
      return 0;
    }
  ...
  static Future<OldMangaClass> getMangaByUUID({required String uuid}) async {
  ...
  static Future<OldMangaClass> getMangaByDir({required String dir}) async {
    ...
    if (response.statusCode == 200) {
      ...
      return OldMangaClass.fromJson(js);
...
const OldMangaClass emptyManga = OldMangaClass(
    uuid: 'MangaNotFound',
    title: 'MangaNotFound',
